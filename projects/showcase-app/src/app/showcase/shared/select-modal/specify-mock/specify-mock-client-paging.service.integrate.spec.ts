...
import { SpecifyMockClientPagingService } from './specify-mock-client-paging.service';

describe('SpecifyMockClientPagingService', () => {
  let srv: SpecifyMockClientPagingService;
  ...
  beforeEach(() => {
    ...
    srv = TestBed.inject(SpecifyMockClientPagingService);
  ...
  it('open開窗後需可返回值', fakeAsync(() => {
    ...
    srv.open([]).subscribe(result => {
      expect(result).toEqual([{ orderId: 'No_000002', orderDate: '2023/06/14', status: 0, customerId: 'C03' }]);
    });
    fixture.detectChanges();
    tick(1000);
