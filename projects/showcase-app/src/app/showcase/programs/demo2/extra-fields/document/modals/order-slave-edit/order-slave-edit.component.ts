...
import { ExtraFieldsDocumentOrderProductInfo } from '../../model/order.model';
...
export class ExtraFieldsDocumentOrderSlaveEditComponent implements OnInit {
  public detailEditForm: FormGroup;
  public detailEdit: ExtraFieldsDocumentOrderProductInfo;
  private _cmd: string;
  ...
  @Input()
  set orderDetail(orderDetail: ExtraFieldsDocumentOrderProductInfo) {
  ...
  ngOnInit(): void {
    if (this._cmd === 'add') {
      this.detailEdit = new ExtraFieldsDocumentOrderProductInfo({});
