...
import { DwDataTable, DwReadService } from '@webdpt/framework/document';
...
import { DocumentOrderDetailComponent } from './order-detail.component';
...
export class DwTabRoutingServiceStub {
  tabRouterChanged$: BehaviorSubject<any> = new BehaviorSubject({
    'componentType': DocumentOrderDetailComponent,
...
describe('DocumentOrderDetailComponent', () => {
  let component: DocumentOrderDetailComponent;
  let fixture: ComponentFixture<DocumentOrderDetailComponent>;
  let de: DebugElement;
  let router: Router;
  ...
  describe('共用條件測試', () => {
    ...
    beforeEach(waitForAsync(() => {
      ...
      spyReading = spyOn(DocumentOrderDetailComponent.prototype, 'reading').and.callThrough();
      spyReaded = spyOn(DocumentOrderDetailComponent.prototype, 'readed').and.callThrough();
    ...
    beforeEach(() => {
      fixture = TestBed.createComponent(DocumentOrderDetailComponent);
    ...
    it('doc.read讀取資料前後, 需先執行IDwDocumentOnRead的reading及readed方法', fakeAsync(() => {
    ...
    it('按下修改(modify), 需導向編輯頁', fakeAsync(() => {
      const modiyfBT: HTMLButtonElement = de.queryAll(By.css('button'))[0].nativeElement;
    ...
    it('按下返回(list), 需導向上一層路由列表頁', fakeAsync(() => {
      const modiyfBT: HTMLButtonElement = de.queryAll(By.css('button'))[1].nativeElement;
      modiyfBT.dispatchEvent(new Event('click'));
      fixture.detectChanges();
      tick();
  ...
  describe('個別條件測試', () => {
    ...
    describe('doc.read錯誤', () => {
      it('返回訊息有errorMessage,, 需執行dwModalService.error, IDwDocumentOnRead的readed方法需不被執行', fakeAsync(() => {
        // ininData在constructor觸發,spy的寫法要改如下......
        const spyInitDataTriggerByConstructor = spyOn(DocumentOrderDetailComponent.prototype, 'initData').and.callThrough();
