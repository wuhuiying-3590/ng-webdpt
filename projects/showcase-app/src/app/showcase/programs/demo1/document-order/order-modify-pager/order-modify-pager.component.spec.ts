...
import { DocumentOrderModifyPagerComponent } from './order-modify.component';
...
export class DwTabRoutingServiceStub {
  tabRouterChanged$: BehaviorSubject<any> = new BehaviorSubject({
    'componentType': DocumentOrderModifyPagerComponent,
...
describe('DocumentOrderModifyPagerComponent', () => {
  let component: DocumentOrderModifyPagerComponent;
  let fixture: ComponentFixture<DocumentOrderModifyPagerComponent>;
  let de: DebugElement;
  let doc: DwDocument;
  let router: Router;
  ...
  describe('共用條件測試', () => {
    ...
    beforeEach(() => {
      fixture = TestBed.createComponent(DocumentOrderModifyPagerComponent);
  ...
  describe('個別條件測試', () => {
      ...
      it('按下確定新增, 需變更單身資料', fakeAsync(() => {
        TestBed.configureTestingModule(commonConfig)
          ...
          .compileComponents().then(() => {
            ...
            tick();
            expect(spyModalCreate).toHaveBeenCalledTimes(1);
            expect(component.detail.demo_orderdetail.controls[2].get('price').value).toEqual(5);
            expect(component.detail.demo_orderdetail.controls[2].get('productid').value).toEqual('productCode1234');
          });
      }));
    ...
    describe('doc.read錯誤', () => {
      it('返回訊息有errorMessage,, 需執行dwModalService.error', fakeAsync(() => {
        ...
        const spyInitDataTriggerByConstructor = spyOn(DocumentOrderModifyPagerComponent.prototype, 'initData').and.callThrough();
