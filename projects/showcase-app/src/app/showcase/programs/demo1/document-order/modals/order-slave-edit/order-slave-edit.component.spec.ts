...
import { DocumentOrderProductInfo } from '../../model';
import { DocumentOrderSlaveEditComponent } from './order-slave-edit.component';
...
export class TestModalComponent {
  constructor(
    ...
    onOk(_data: DocumentOrderSlaveEditComponent): void {
    }
    cancel(): void {}
    modalRef: NzModalRef;
    open(state: string): void {
      this.modalRef = this.dwModalService.create({
        ...
        nzComponentParams: state === 'modify' ? {
          cmd: state,
          orderDetail: new DocumentOrderProductInfo(this.modifyData)
        } : { cmd: state }
      });
    }

}
describe('DocumentOrderSlaveEditComponent', () => {
  ...
  beforeEach(waitForAsync(() => {
    TestBed.configureTestingModule({
      ...
      providers: [
        Overlay,
        NzModalService],
      declarations: [DocumentOrderSlaveEditComponent, TestModalComponent]
    })
      .compileComponents();
  }));
  ...
  describe('當開窗修改時', () => {
    ...
    it('按下確認, 需執行modalSubject.triggerOk, 返回DocumentOrderSlaveEditComponent給dwOnOk', fakeAsync(() => {
      testComponent.open('modify');
      fixture.detectChanges();
      ...
      fixture.detectChanges();
      tick(1000);
      expect(spyOnOk.calls.mostRecent().args[0] instanceof DocumentOrderSlaveEditComponent).toBeTrue();
    }));
  ...
  describe('當開窗新增時', () => {
    it('按下確認, 需執行modalSubject.triggerOk, 返回DocumentOrderSlaveEditComponent給dwOnOk', fakeAsync(() => {
      ...
      fixture.detectChanges();
      // 加資料
      (testComponent.modalRef.componentInstance as DocumentOrderSlaveEditComponent)
      ...
      fixture.detectChanges();
      tick(1000);
      expect((spyOnOk.calls.mostRecent().args[0] as DocumentOrderSlaveEditComponent).detailEdit.productCode).toEqual('id9999');
    }));
  });
});
