...
export class CanvasDataService {
  ...
  cloneComponentWithOrigin(origin) {
    let newComponent;
    console.log(origin);
    for (const key in origin) {
      if (origin.hasOwnProperty(key)) {
        ...
        // const component=this.createNewComponent(origin.type)
        if (origin.children && Array.isArray(origin.children)) {
          for (let i = 0; i < origin.children.length; i++) {
            this.cloneComponentWithOrigin(origin.children[i]);
          }
        }
      }
    }
  ...
  cloneComponent(component: any) {
    ...
    this.cloneComponentWithOrigin(component);
  }
}
