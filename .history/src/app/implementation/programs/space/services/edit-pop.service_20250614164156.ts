...
export class EditPopService implements OnDestroy {
  ...
  isElementInHorizontalView(container: HTMLElement, element: HTMLElement) {
    ...
    return (
      elementOffsetLeft >= containerScrollLeft &&
      elementOffsetLeft + elementWidth <= containerScrollLeft + containerWidth
    );
  }
  ...
  changeSelectBorderPosition(target) {
    const flag = this.isElementInHorizontalView(target, this.selectEl);
