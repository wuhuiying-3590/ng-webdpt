...
export class CanvasDataService {
  ...
  // 删除组件（默认删除选中组件）
  removeComponent(component: any = this.selectedComponentSubject.value) {
    const treeData = this.treeDataSubject.value;
    // 递归查找并更新
    function removeNode(nodes: any[]): boolean {
      for (let i = 0; i < nodes.length; i++) {
  ...
  getParentComponentId(component: any) {
    const treeData = this.treeDataSubject.value;
    function getParentIdInTreeData(component, pId = 'root') {
      for (const node of treeData) {
        // node={id:'xxx',children:[]}
        if (node.id === component.id) {
          return pId;
        } else {
          if (node.children && Array.isArray(node.children)) {
            const _pId = node.id;
            return getParentIdInTreeData(node.children, _pId);
          }
        }
      }
    }
    const pId = getParentIdInTreeData(component);
    console.log('getParentComponentId', pId);
  }
  // 复制组件（默认复制选中组件）
  cloneComponent(
    component: any = this.selectedComponentSubject.value,
    pId: string
  ) {
    // console.log(this.selectedComponentSubject.value, '-----');
    // console.log(component, '-----');
    console.log('pId', pId);
    this.getParentComponentId(component);
    return;
