...
export class CanvasDataService {
  ...
  cloneComponent(component: any, pId: string) {
   ...
   findAndReplaceTitle:(components: any[], componentToInsert: any): void=()=> {
    for (let i = 0; i < components.length; i++) {
      if (components[i].type === 'title') {
        const titleComponent = JSON.parse(JSON.stringify(components[i]));
        components.splice(i, 1, componentToInsert);
        return titleComponent;
      }
      if (components[i].children && components[i].children.length > 0) {
        this.findAndReplaceTitle(components[i].children, componentToInsert);
      }
    }
  }
  changeTitlePosition = () => {
    console.log('changeTitlePosition');
    const treeData = this.treeDataSubject.value;
    const firstComponent = treeData[0];
    treeData.shift();
    const titleComponent = this.findAndReplaceTitle(treeData, firstComponent);
    this.treeData.unshift(titleComponent);
    console.log('findAndReplaceTitle', this.treeData);
  };
