...
export class CanvasDataService {
  ...
  addComponent(
    ...
    pId = this.selectedComponentSubject.value?.id,
    cIndex?: number
  ) {
    ...
    if (!pId || pId === 'root') {
      ...
      const pType = this.selectedComponentSubject.value?.type;
      this.addComponentInTreeData(newComponent, pId, cIndex, true);
    }
  }
  ...
  // 复制组件（默认复制选中组件）
  cloneComponent(component: any = this.selectedComponentSubject.value) {
