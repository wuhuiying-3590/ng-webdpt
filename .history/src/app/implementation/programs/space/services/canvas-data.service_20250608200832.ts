...
export class CanvasDataService {
  ...
  // romoveLayoutComponent() {
  //   const treeData = this.treeDataSubject.value;

  removeEmptyScrollLayouts = (nodes: any[]): boolean => {
    ...
    for (let i = nodes.length - 1; i >= 0; i--) {
      ...
      // 首先递归检查子节点
      if (node.children && Array.isArray(node.children)) {
        if (this.removeEmptyScrollLayouts(node.children)) {
          hasChanged = true;
        }
      }
  ...
  //   if (removeEmptyScrollLayouts(treeData)) {
  //     this.treeDataSubject.next([...treeData]); // 发送新的数组引用以触发变更检测
  //   }
  // }
  // 其他操作（如删除、更新等）可按需添加
  removeComponent(component: any) {
    ...
    if (removeNode(treeData)) {
      this.removeEmptyScrollLayouts(treeData);
