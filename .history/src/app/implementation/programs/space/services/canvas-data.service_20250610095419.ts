...
export class CanvasDataService {
        ...
        for (const node of nodes) {
          if (node.id === selectedComponentId) {
            if (componentType === 'title') {
              ...
              if (selectedComponentId.includes('title')) {
                msg = '只可以添加一个标题组件';
              }
  ...
  updateComponent(
    ...
    const treeData = this.treeDataSubject.value;
    let copyComponent;
    function copyComponentInRoot(root) {
      if (fromId === 'root') {
    ...
    copyComponentInRoot(rootData);
    copyComponentInTree(treeData);
    console.log('copyComponent', copyComponent);
    // if (updateSortInTree(treeData)) {
    // console.log('updateSortInTree', treeData);
    // this.treeDataSubject.next(treeData); // 通知所有订阅者
    // }
  }
  // 更新指定组件的 style
