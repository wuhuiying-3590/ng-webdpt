...
export class ComponentDataService {
  ...
  getComponentChildStyles(component: any): any {
    ...
    if (style.mode) {
      if (style.mode === 'fixed') {
        if (style.fixedSize && style.fixedSize !== 'customize') {
          result.width = FIXEDSIZE_STYLE[style.fixedSize].width;
          result.height = FIXEDSIZE_STYLE[style.fixedSize].height;
        } else {
          result.width = style.customWidth;
          result.height = style.customHeight;
        }
      }
    }
    return result;
