...
export class CanvasDataService {
  ...
  addComponentWithData(
    component: any,
    pId: string | undefined = this.selectedComponentSubject.value?.id,
    cIndex?: number
  ) {
    console.log('addComponentWithData:', component);
    if (!pId || pId === 'root') {
      this.addComponentInRoot(JSON.parse(component), cIndex);
    } else {
      this.addComponentInTreeData(JSON.parse(component), pId, cIndex);
    }
  }
  ...
  // removeSortableItem(componentId?: string) {
  //   const treeData = this.treeDataSubject.value;
  //   // 递归查找并更新
  //   function removeNode(nodes: any[]): boolean {
  //     for (let i = 0; i < nodes.length; i++) {
  //       if (!nodes[i]?.id || nodes[i]?.id === componentId) {
