...
export class CanvasDataService {
  ...
  updateComponent(
    ...
    const treeData = this.treeDataSubject.value;
    console.log('updateComponent', treeData); // 数据已经是调整嵌套顺序后的

    let copyComponent;
    function copyComponentInRoot() {
      if (fromId === 'root') {
        copyComponent = treeData[oldIndex];
      }
    }
    function copyComponentInTree(nodes) {
      for (const node of nodes) {
        if (node.id === fromId) {
          copyComponent = node.children[oldIndex];
          return true; // 找到后返回
        }
        if (node.children && Array.isArray(node.children)) {
          if (copyComponentInTree(node.children)) {
            return true;
          }
        }
      }
  ...
  cloneComponent(component: any) {
    console.log('cloneComponent', component);
    function cloneComponentWithOrigin(origin) {
      const { type, style, children } = origin;
      let newComponent = this.createNewComponent(type, { style });
      if (children && Array.isArray(children)) {
        for (const node of children) {
          cloneComponentWithOrigin(node);
        }
      }
    }
    cloneComponentWithOrigin(component);
    // if(cloneComponentWithOrigin())
  }
}
