...
export class CanvasDataService {
  ...
  updateSelectedComponentProperty(property: any) {
    const treeData = this.treeDataSubject.value.slice();
    const selectedComponent = { ...this.selectedComponentSubject.value };
    const updatedArray = this.dateUtilsService.updatePropertyInNestedArray(
      treeData,
    ...
    this.treeDataSubject.next(updatedArray);
  }
}
