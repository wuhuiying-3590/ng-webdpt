...
export class WidgetComponent implements OnInit, OnDestroy {
  ...
  saveAsTemplate(): void {
    ...
    this.httpServerService
      .saveTemplate({ id: this.id, webStyleData: JSON.stringify(templateData) })
      .subscribe((canvasData: any) => {
  ...
  getCanvasData(id) {
    ...
    this.httpServerService.getTemplateData(id).subscribe((canvasData: any) => {
      const webStyleData = JSON.parse(canvasData.webStyleData);
      console.log('webStyleData:', webStyleData);
      // 2. 设置 root 和 treeData
      this.canvasDataService.setRoot(webStyleData.root);
