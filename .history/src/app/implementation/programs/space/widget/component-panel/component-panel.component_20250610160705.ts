...
export class ComponentPanelComponent implements OnInit {
  ...
  componentPanelSortableOptions = {
    ...
    //   dataTransfer.setData('Text', '你好'); // `dataTransfer` object of HTML5 DragEvent
    // },
    onClone: (evt) => {
      const item = evt.item;
      let cloneEl = evt.clone;
      const componentType = item.getAttribute('data-component-type');
      cloneEl = this.canvasDataService.createNewComponent('title');
      console.log('onClone', evt, componentType, cloneEl);
    },
    onMove: (evt) => {
