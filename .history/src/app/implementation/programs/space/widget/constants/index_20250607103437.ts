...
//   return origin;
// };
// export const lineSpaceOption = getLineSpaceOptions([...baseOption]);
...
export const getLineSpaceOptions = (type: string) => {
  switch (type) {
    case ComponentType.ROOT:
      const ROOT_OPTION = [...baseOption];
      ROOT_OPTION.splice(1, 1);
      ROOT_OPTION.push({
        label: '28px',
        value: '28px',
      });
      return ROOT_OPTION;
    case ComponentType.SINGLE_COLUMN || ComponentType.MULTI_COLUMN:
      const SINGLE_COLUMN_OPTION = [...baseOption];
      SINGLE_COLUMN_OPTION.splice(1, 1);
      return SINGLE_COLUMN_OPTION;
    default:
      return [];
  }
};
