...
export class ListItemComponent implements OnInit {
  ...
  componentSortablejsOptions = {
    ...
    onStart: (evt) => {
      console.log('onStart---componentSortablejsOptions');
      ...
      let imgSrc = componentList.find((el) => el.type === componentType).icon; // 默认图片

      // 根据 componentType 设置不同的图片，这里只是示例，你需要根据实际情况调整
      if (componentType === 'layout-single-column') {
        imgSrc = 'assets/images/layout-single-column.png';
      } else if (componentType === 'layout-multi-column') {
        imgSrc = 'assets/images/layout-multi-column.png';
      }
      // ... 其他组件类型
