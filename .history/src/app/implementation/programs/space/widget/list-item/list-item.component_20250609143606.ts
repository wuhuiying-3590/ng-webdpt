...
export class ListItemComponent implements OnInit {
  ...
  slotSortablejsOptions = {
    ...
    onAdd: (event: any) => this.onAdd(event),
    // filter: '.no-drag',
  };
  ...
  onAdd(event: any) {
    ...
    const parentComponent = this.components[event.newDraggableIndex]; // 获取父组件

    if (parentComponent && parentComponent.children) {
      // 将新组件添加到父组件的 children 数组中
      parentComponent.children.splice(event.newIndex, 0, newComponentData);
    }
    // 移除拖拽的 DOM 元素，因为它已经被添加到数据模型中，Angular 会重新渲染
    event.item.remove();
  }
}
