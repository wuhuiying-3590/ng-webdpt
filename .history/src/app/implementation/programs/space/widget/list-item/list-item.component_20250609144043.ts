...
export class ListItemComponent implements OnInit {
  ...
  sortablejsOptions = {
    group: { name: 'share', pull: true, put: false },
  };

  // Modified: Make slotSortablejsOptions a function
  getSlotSortablejsOptions(parentComponent: any): any {
    return {
      group: { name: 'share', pull: false, put: true },
      onAdd: (event: any) => this.onSlotAdd(event, parentComponent),
      // filter: '.no-drag',
    };
  }

  // New handler for slot-specific adds
  onSlotAdd(event: any, parentComponent: any) {
    console.log('onSlotAdd event:', event);
    const newComponentData = JSON.parse(event.item.dataset.component);
    console.log('newComponentData:', newComponentData);

    // Remove the dragged DOM element from the view. Angular will re-render from data.
    // This is important because Sortable.js directly adds the DOM element.
    event.item.remove();
    ...
    // Add the new component data to the parent's children array at the correct index
    parentComponent.children.splice(event.newIndex, 0, newComponentData);
