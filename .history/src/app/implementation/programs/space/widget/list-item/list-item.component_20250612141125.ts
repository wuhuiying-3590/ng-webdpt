...
export class ListItemComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    document.addEventListener('mousemove', (evt) => {
      if (this.customDragEl) {
        console.log('mousemove', evt);
        const width = Math.floor(this.customDragEl.offsetWidth / 2);
        const height = Math.floor(this.customDragEl.offsetHeight / 2);
        this.customDragEl.style.left = evt.clientX - width + 'px';
        this.customDragEl.style.top = evt.clientY - height + 'px';
      }
    });
  }
  ...
  componentSortablejsOptions = {
    ...
    onStart: (evt) => {
      ...
      // console.log(width);
      // 初始位置
      this.customDragEl.style.left = evt.originalEvent.clientX - width + 'px';
      this.customDragEl.style.top = evt.originalEvent.clientY - height + 'px';
    },
    onMove: (evt) => {
      // console.log(evt);
