...
slotContentSortablejsOptions={
  ...
  getSlotSortablejsOptions(parentComponent: any): any {
    return {
      group: { name: 'share', pull: false, put: false },
  ...
  // Existing: SortableJS options for content inside a slot-component
  getSlotContentSortablejsOptions(slotComponent: any): any {
    return {
      group: { name: 'share', pull: true, put: true },
      // onAdd: (event: any) => this.onSlotContentAdd(event, slotComponent),
      // filter: '.no-drag',
    };
  }

  // Existing: Handler for adding components inside a slot-component
  onSlotContentAdd(event: any, slotComponent: any) {
    // ... existing code ...
  }
}
