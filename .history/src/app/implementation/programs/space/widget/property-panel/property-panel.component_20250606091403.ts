...
export class PropertyPanelComponent implements OnInit, OnDestroy {
  ...
  backgroundTypeChange(type: string) {
    if (type === 'transparent') {
      ...
      // 默认选第一个颜色
      const firstColor = this.chooseColorOption[0]?.value;
      this.propertyForm.get('backgroundColor')?.setValue(firstColor);
    }
  }

  backgroundColorChange(color: string) {
    // 选中颜色时，把颜色值赋给 backgroundType
    // this.propertyForm.get('backgroundType')?.setValue(color);
  }
  ngOnInit(): void {
    this.initData();
    ...
    this.propertyForm.valueChanges
      ...
      .subscribe((value) => {
        ...
        // this.canvasDataService.updateSelectedComponentProperty(value);
      });
  }
  ngOnDestroy(): void {
    this.destroy$.next();
