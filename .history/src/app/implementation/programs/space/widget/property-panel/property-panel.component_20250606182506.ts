...
export class PropertyPanelComponent implements OnInit, OnDestroy {
  ...
  isInitSelectedComponentStyle = false;
  ...
  loadComponentProperties(): void {
    this.isInitSelectedComponentStyle = true;
    // 先重置表单，清空之前的值
  ...
  ngOnInit(): void {
    ...
    this.propertyForm.valueChanges
      ...
      .subscribe((value) => {
        if (this.isInitSelectedComponentStyle) {
          this.isInitSelectedComponentStyle = false;
          return;
        }
        this.backgroundTypeChange(value.backgroundType);
