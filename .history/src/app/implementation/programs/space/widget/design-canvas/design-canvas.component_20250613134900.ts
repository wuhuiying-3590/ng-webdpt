...
export class DesignCanvasComponent implements OnInit {
  ...
  // 选择组件
  selectComponentItem(event: MouseEvent, component: any, selectEl?): void {
    // 阻止事件冒泡
    ...
    if (
      ...
      this.canvasDataService.setSelectEl(selectEl);
  ...
  getOffsetRelativeToContainer(child: HTMLElement, container: HTMLElement) {
    ...
    console.log(child, container, container.scrollLeft, container.scrollTop);
    return {
  ...
  changeSelectBorderPosition() {
    console.log('changeSelectBorderPosition', this.canvasDataService.selectEl);
    const element = this.canvasDataService.selectEl;
    console.log(this.designCanvasRef.nativeElement);
